...
今回の開発では訳あって採用されなかった技術についても、今後の採用を見越し、qiite で記事として記述してスキルの定着を図りました
...
- 実際に触ってみて簡単だった
  ...
  Axios は設定が直感的で、実際にプロジェクトで使用した際にストレスなく扱えると感じました。シンプルな API 設計により、今後のユーザー認証などの機能の実装の際に、`fetch`での連携よりも開発効率が高いと判断し、保守性も向上することを期待しています。
